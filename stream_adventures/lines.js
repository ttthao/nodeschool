...
process.stdin
    ...
    .pipe(through(function(buffer, _, next) {
        // count % 2 ? console.dir(line.toString().toUpperCase()) : console.dir(line.toString().toLowerCase())

        // split will split lines on newline, so add it back in to keep originality
        this.push(count % 2 ? buffer.toString().toUpperCase()+"\n" : buffer.toString().toLowerCase()+"\n")
        ++count
        next()
    }))
    .pipe(process.stdout)