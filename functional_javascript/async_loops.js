...
## Arguments
  ...
  * load: a Function used to load user objects. Expects a numeric id and a callback.
  The callback will be called with the result of loading the user with the 
  specified id (either a user object or null).
...
module.exports = function loadUsers(userIds, load, done) {
    // Iterate id's with map to call load 
    var users = userIds.map(function(id) {
        return load(id);
    });
    // Callback done with user from load
    done(users);
}