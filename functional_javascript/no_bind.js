...
As an example, say we have a function add, that takes 2 arguments and adds them
together:
...
addTen is a new function that takes the y parameter of add. add has not yet been
called!
...
All of the above examples are same as calling add(10, y), where y was supplied
in the call to the appropriately named addTen.
...
Function.prototype.apply allows us to execute a function, supply a new 'value
for this' (we can ignore in this case), and then an array of arguments to apply to the function:
...
module.exports = function logger(namespace) {
    // console.log(arguments) // It will be INFO and WARN during declaration/initialization
    return function () { // Returning a function won't invoke it
        // console.log(arguments) // It will consist of the arguements passed during invocation
        console.log.apply(console, [namespace].concat(slice.call(arguments))) // Why can't this be null
      }
}
