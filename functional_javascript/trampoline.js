...
The boilerplate includes a definition of repeat. repeat will take a Function
operation, and a Number num, and invoke operation num times:
...
You can assume that the operation passed to repeat does not take arguments (or
  they are already bound to the function) and the return value is not important.
...
function repeat(operation, num) {
  ...
  // The next step is num decrements, and oepration gets called
  // If num <= 0 
  if (num <= 0) return
  return function() {
    operation()
...
module.exports = function (operation, num) {
    var _trampoline = trampoline(function() {
      return repeat(operation, num)
    })
    return _trampoline
}
